...
## 13.2 Monte Carlo simulations for categorical date
...
```{r}
draw <- sample(balls, C, replace = FALSE)
prop.table(table(draw))
```
...
```{r}
draw <- sample(balls, C, replace = TRUE)
prop.table(table(draw))
```
...
```{r}
game <- sample(c("Celtics", "Cavs"), 7, replace = TRUE, prob = c(0.4, 0.6))
prop.table(table(game))
```
