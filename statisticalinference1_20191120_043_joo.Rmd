...
We start by defining the random variable $X$ as: $X=1$ if we pick a blue bead at random and $X=0$ if it is red. This implies that the population is a list of 0s and 1s. If we sample $N$ beads, then the average of the draws $X_1,…,X_N$is equivalent to the proportion of blue beads in our sample. This is because adding the $X$s is equivalent to counting the blue beads and dividing this count by the total $N$ is equivalent to computing a proportion. We use the symbol $\overline{X}$to represent this average. In general, in statistics textbooks a bar on top of a symbol means the average. The theory we just learned about the sum of draws becomes useful because the average is a sum of draws multiplied by the constant $1/N$:
...
To understand how good our estimate is, we will describe the statistical properties of the random variable defined above: the sample proportion $\overline{X}$. Remember that $\overline{X}$ is the sum of independent draws so the rules we covered in the probability chapter apply.

Using what we have learned, the expected value of the sum $N\overline{X}$ is $N×$ the average of the urn, $p$. So dividing by the non-random constant $N$ gives us that the expected value of the average $\overline{X}$ is $p$. We can write it using our mathematical notation:
$$E(\overline{X})=p$$

We can also use what we learned to figure out the standard error: the standard error of the sum is $\sqrt{N}×$ the standard deviation of the urn. Can we compute the standard error of the urn? We learned a formula that tells us that it is $(1−0)\sqrt{p(1−p)} = \sqrt{p(1−p)}$. Because we are dividing the sum by $N$, we arrive at the following formula for the standard error of the average:
$$SE(\overline{X})=\sqrt{p(1−p)/N}$$

This result reveals the power of polls. The expected value of the sample proportion $\overline{X}$ is the parameter of interest $p$ and we can make the standard error as small as we want by increasing $N$. The law of large numbers tells us that with a large enough poll, our estimate converges to $p$.
...
or 1.5 percentage points. So even with large polls, for close elections, $\overline{X}$ can lead us astray if we don’t realize it is a random variable. Nonetheless, we can actually say more about how close we get the $p$ and we do that in Section 15.4.
...
3. Consider the random variable $S/N$. This is equivalent to the sample average, which we have been denoting as $\overline{X}$. What is the expected value of the $\overline{X}$? Hint: it’s a function of $p$.
```{r}
p
```

4. What is the standard error of $\overline{X}$? Hint: it’s a function of $p$.
```{r}
# SE(¯X) = sqrt(p*(1-p)/N)
```
...
7. If we are interested in the difference in proportions, $p−(1−p)$, our estimate is $d=\overline{X}−(1−\overline{X})$. Use the rules we learned about sums of random variables and scaled random variables to derive the expected value of $d$.
```{r}
...
```{r}
2*sqrt(0.45 * (1-0.45) / 25)
```

10. Given the answer to 9, which of the following best describes your strategy of using a sample size of $N=25$?

a. The expected value of our estimate $2\overline{X}-1$ is $d$, so our prediction will be right on.
...
The CLT tells us that the distribution function for a sum of draws is approximately normal. We also learned that dividing a normally distributed random variable by a constant is also a normally distributed variable. This implies that the distribution of $\overline{X}$ is approximately normal.

In summary, we have that $\overline{X}$ has an approximately normal distribution with expected value $p$ and standard error $\sqrt{p(1−p)/N}$.
...
$$Pr(\overline{X}≤p+.01)−Pr(\overline{X}≤p−.01)$$

Can we answer this question? We can use the mathematical trick we learned in the previous chapter. Subtract the expected value and divide by the standard error to get a standard normal random variable, call it $Z$, on the left. Since $p$ is the expected value and $SE(\overline{X})=\sqrt{p(1−p)/N}$ is the standard error we get:
$$Pr(Z≤\frac{.01}{SE(\overline{X})})−Pr(Z≤−\frac{.01}{SE(\overline{X})})$$

One problem we have is that since we don’t know $p$, we don’t know $SE(\overline{X})$. But it turns out that the CLT still works if we estimate the standard error by using $\overline{X}$ in place of $p$. We say that we *plug-in* the estimate. Our estimate of the standard error is therefore:
...
Now we continue with our calculation, but dividing by $\hat{SE}(\overline{X})=\sqrt{\overline{X}(1−\overline{X})/N}$ instead. In our first sample we had 12 blue and 13 red so $\overline{X}=0.48$ and our estimate of standard error is:
```{r}
x_hat <- 0.48
...
$$Pr(Z≤1.96SE(\overline{X})/SE(\overline{X}))−Pr(Z≤−1.96SE(\overline{X})/SE(\overline{X}))$$
...
Hence, there is a 95% probability that $\overline{X}$ will be within $1.96×\hat{SE}(\overline{X})$, in our case within about 0.2, of $p$. Note that 95% is somewhat of an arbitrary choice and sometimes other percentages are used, but it is the most commonly used value to define margin of error. We often round 1.96 up to 2 for simplicity of presentation.
...
From the table above, we see that typical sample sizes range from 700 to 3500. To see how this gives us a much more practical result, notice that if we had obtained a $\overline{X}$=0.48 with a sample size of 2,000, our standard error $\hat{SE}(\overline{X})$ would have been 0.011. So our result is an estimate of `48`% with a margin of error of 2%. In this case, the result is much more informative and would make us think that there are more red balls than blue. Keep in mind, however, that this is hypothetical. We did not take a poll of 2,000 since we don’t want to ruin the competition.
...
To review, the theory tells us that $\overline{X}$ is approximately normally distributed, has expected value $p=$ 0.45 and standard error $\sqrt{p(1−p)/N}$= 0.016. The simulation confirms this:
```{r}
mean(x_hat)
sd(x_hat)
```
...
The competition is to predict the spread, not the proportion $p$. However, because we are assuming there are only two parties, we know that the spread is $p−(1−p)=2p−1$. As a result, everything we have done can easily be adapted to an estimate of $2p−1$. Once we have our estimate $\overline{X}$ and $\hat{SE}(\overline{X})$, we estimate the spread with $2\overline{X}−1$ and, since we are multiplying by 2, the standard error is $2\hat{SE}(\overline{X})$. Note that subtracting 1 does not add any variability so it does not affect the standard error.
...
3. The vector `errors` contains, for each simulated sample, the difference between the actual $p$ and our estimate $\overline{X}$. We refer to this difference as the *error*. Compute the average and make a histogram of the errors generated in the Monte Carlo simulation and select which of the following best describes their distributions:
```{r}
mean(errors)
hist(errors)
```
...
4. The error $\overline{X}−p$ is a random variable. In practice, the error is not observed because we do not know $p$. Here we observe it because we constructed the simulation. What is the average size of the error if we define the size by taking the absolute value $∣\overline{X}−p∣$?
```{r}
mean(abs(errors))
```
...
6. The theory we just learned tells us what this standard deviation is going to be because it is the standard error of $\overline{X}$. What does theory tell us is the standard error of $\overline{X}$ for a sample size of 100?
```{r}
sqrt(p*(1-p)/N)
```

7. In practice, we don’t know $p$, so we construct an estimate of the theoretical prediction based by plugging in $\overline{X}$ for $p$. Compute this estimate. Set the seed at 1 with `set.seed(1)`.
```{r}
set.seed(1)
...
9. For sample size $N=100$, the central limit theorem tells us that the distribution of $\overline{X}$ is:
...
10. Based on the answer from exercise 8, the error $\overline{X}−p$ is:
...
12. If $p=0.45$ and $N=100$ as in exercise 2, use the CLT to estimate the probability that $\overline{X}>0.5$. You can assume you know $p=0.45$ for this calculation.
```{r}
p <- 0.45
N <- 100
...
13. Assume you are in a practical situation and you don’t know $p$. Take a sample of size $N=100$ and obtain a sample average of $\overline{X}=0.51$. What is the CLT approximation for the probability that your error is equal to or larger than 0.01?
```{r}
N <-100
X_ <- 0.51
se_ <- sqrt(X_*(1-X_)/N)

1 - pnorm(0.01, 0, se_) + pnorm(-0.01, 0, se_)
```
